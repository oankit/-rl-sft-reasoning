...
def extract_boxed(text: Optional[str]) -> Optional[str]:
    ...
    # extract the last numerical value in the text
...
def load_model_and_tokenizer(model_name: str, device: Optional[str] = None, dtype: Optional[torch.dtype] = None):
    ...
    Args:
        ...
        device: Default (cuda, cpu),
        dtype: Default (fp16, bf16, fp32),
...
# in the paper for mean ablation.
...
def get_decoder_layers(model) -> List[torch.nn.Module]:
    ...
    # Try common architectural patterns for layer access for gpt style models
    layer_paths = [
        ["model", "layers"],
        ["transformer", "h"],
...
def compute_layer_metrics(
    ...
    *,
    ...
    # for each layer's ablation results
    for L, ablated_boxed in enumerate(ablated_boxed_per_layer):
        ...
        # FOC Metric: Among baseline-correct items, how many become incorrect after ablation?
                ...
                if metrics:
                    ...
                    if valid_ad and len(valid_ad) > 10:
                        ...
                        if ad_std > 0.05:
                            ...
                            print(f"Low variation pattern (σ = {ad_std:.3f})")
                else:
                    print("No valid layer metrics computed")
            else:
